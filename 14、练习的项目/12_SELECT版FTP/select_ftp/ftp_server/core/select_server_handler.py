        ...
        while True:
            ...
            for r in readable:
                if r is self.server:
                    ...
                    cmd, response = ftp.get_response()
                    ftp.sendall_data(cmd, response)
                    # personal_ftp = {"obj": ftp, "cmd": cmd, "response": response}
                    # self.queue_dict[r].put(personal_ftp)
                    # outputs.append(r)  # 放入需返回数据的连接的列表中

            # 要返回给客户端的连接列表
            # for w in outputs:
            #     personal_ftp = self.queue_dict[w].get()
            #     personal_ftp["obj"].sendall_data(personal_ftp["cmd"], personal_ftp["response"])
            #     outputs.remove(w)
