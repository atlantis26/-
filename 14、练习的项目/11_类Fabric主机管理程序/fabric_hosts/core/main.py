...
class _FabricHost(object):
    ...
    @staticmethod
    def delete():
        host_id = input(u"请输入主机的Id号：").strip()
    ...
    def execute():
        try:
            host_id = input(u"请输入远程目标主机的Id号：").strip()
            ...
            print(u"执行失败，原因：登录失败，请检查目标主机的连接配置信息中账号密码是否正确")
        except TimeoutError:
            print(u"执行失败，原因：连接超时，请检查目标主机的网络是否连通")
    ...
    def put():
        try:
            ...
            if rsp.code == 200:
                ...
                print(u"主机Id号{0}不存在，请核对后再试".format(host_id))
        except paramiko.ssh_exception.AuthenticationException:
            print(u"执行失败，原因：登录失败，请检查目标主机的连接配置信息中账号密码是否正确")
        except TimeoutError:
            print(u"执行失败，原因：连接超时，请检查目标主机的网络是否连通")
...
class _FabricHostGroup(object):
    ...
    def create():
        ...
        group_ids = input(u"请输入添加为成员的主机的Id号（多个主机id以逗号连接）：").strip()
        rsp = HostGroupHandler.create(group_name, group_ids)
        print(rsp.msg)

    @staticmethod
    def delete():
        group_id = input(u"请输入主机组的Id号：").strip()
    ...
    def modify():
        ...
        add_hosts = input(u"请输入新添加的成员主机的Id号（多个主机id以逗号连接）：").strip()
        delete_hosts = input(u"请输入移出的成员主机的Id号（多个主机id以逗号连接）：").strip()
        rsp = HostGroupHandler.modify(group_id, group_name, add_hosts, delete_hosts)
        print(rsp.msg)
    ...
    @staticmethod
    def execute():
        group_id = input(u"请输入目标主机组的Id号：").strip()
        commands = input(u"请输入需要执行的命令（可执行多个命令，命令以逗号连接）：").strip()
        rsp = HostGroupHandler.detail(group_id)
        if rsp.code == 200:
            ...
            print(u"主机组Id号{0}不存在，请核对后再试".format(group_id))
...
class Fabric(object):
    ...
    @staticmethod
    def console():
        print(u"------------------欢迎使用Fabric运维管理系统-----------------")
        while True:
            info = u"""
    您可以选择如下的操作：
        1.显示主机列表                                          2.显示主机组列表
        3.进入主机信息管理视图                                  4.进入主机组信息管理视图
        5.执行命令                                              6.上传文件
        7.下载文件                                              8.退出系统
            """
            print(info)
            action = input(u"请输入操作的数字编号：").strip()
