...
def save_host_group(host_group):
    ...
    if os.path.exists(json_file):
        raise SomeError("id编号为{0}的主机组已存在".format(host_group.group_id))
    with open(json_file, "w") as f:
...
def get_new_host_id():
    """新增时，生成新主机id编号，从1000开始计数"""
    return str(1000 + len(list_hosts()))


def get_new_host_group_id():
    """新增时，生成新主机组id编号，从1000开始计数"""
    return str(1000 + len(list_host_groups()))
